...
class Enemy extends rect {
  ...
  public void draw() {
    if (entry != null) {
      ...
      if (PApplet.parseInt(random((1000-level) * frameRate/60)) == 0){
...
public void updatePosByWebcam() {
  ...
  if (faces.length > 0) {
    ...
    player.cords.x = width - Math.round(faces[0].x*6.5f);
...
class SortBySize implements Comparator<Rectangle> {
  public int compare(Rectangle a, Rectangle b) {
    return PApplet.parseInt(a.width*a.height*1000)-PApplet.parseInt(b.width*b.height*1000);
  }
}
  public void settings() {  size(672, 864); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "shooter" };
    if (passedArgs != null) {
